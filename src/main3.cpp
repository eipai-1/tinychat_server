...
http::message_generator handle_request(beast::string_view doc_root,
    ...
    if (req.target() == "/api/time") {
        if (req.method() != http::verb::get) {
            // std::cout << "todo: method not allowed" << std::endl;
            http::response<http::string_body> res{http::status::method_not_allowed, req.version()};
            res.set(http::field::server, BOOST_BEAST_VERSION_STRING);
            // 在 Allow 头部中告诉客户端，这个端点只接受 GET
        ...
        // 这是一个 API 请求，专门处理它
        // 1. 创建一个 Boost.JSON 对象
        boost::json::object jv;

        // 2. 填充 JSON 数据
        auto const now = std::chrono::system_clock::now();
        auto const c_now = std::chrono::system_clock::to_time_t(now);
        // std::put_time 在某些编译器下可能需要特别处理，这里用简单的 asctime
        jv["server_time"] = std::asctime(std::localtime(&c_now));
        jv["status"] = "ok";
        // 3. 将 JSON 对象序列化为字符串
        std::string json_string = boost::json::serialize(jv);

        // 4. 创建一个 string_body 类型的 HTTP 响应
        ...
        // 7. 准备 payload（计算 Content-Length 等）
        res.prepare_payload();
